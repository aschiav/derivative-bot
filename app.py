import os, time, json, random, math, base64, requests
from flask import Flask, request, jsonify, session

# ── Math imports (SymPy) ───────────────────────────────────────────────────────
from sympy import (
    symbols, sympify, simplify, diff, lambdify, pi, E, oo
)
...
from sympy import log, sqrt, Abs

# ── Flask setup ────────────────────────────────────────────────────────────────
app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET", "change-me")  # sessions per student
app.config.update(
    SESSION_COOKIE_SAMESITE="None",
    SESSION_COOKIE_SECURE=True  # required for iframes on HTTPS (Canvas)
)
...
VISION_MODEL = os.environ.get("VISION_MODEL", "gpt-4o-mini")  # vision-capable model

# ── Helpers: per-student thread (not strictly needed here, but handy) ─────────
def ensure_thread():
    if "thread_id" not in session:
        # If you ever want to chat with an Assistant as well, keep this.
...
def allowed_locals(varname="x"):
    x = symbols(varname)
    # Add common symbols/functions you want to allow
    return {
        varname: x,
        "x": x, "y": symbols("y"),  # accept x or y if OCR guessed wrong
        "pi": pi, "E": E,
...
def parse_sympy(expr_str, varname="x"):
    # Safely parse a SymPy-style string into a SymPy expression
    env = allowed_locals(varname)
    return sympify(expr_str, locals=env)

def numeric_equiv(f_expr, g_expr, varname="x"):
    """Heuristic numeric check on a few points to avoid false mismatches."""
    x = symbols(varname)
    f = lambdify(x, simplify(f_expr), "mpmath")
    g = lambdify(x, simplify(g_expr), "mpmath")

    # Try a spread of points; skip those that error or are undefined
    ...
    for t in candidates:
        try:
            fv = f(t); gv = g(t)
            if any(map(lambda v: (v is None) or (isinstance(v, float) and (math.isnan(v) or math.isinf(v))), [fv, gv])):
                continue
    ...
    # If we had at least 4 usable test points and most matched, call it equal
    if tested >= 4 and equal >= max(3, int(0.8 * tested)):
        return True, {"tested": tested, "matched": equal}
    return False, {"tested": tested, "matched": equal}

# ── Vision OCR via OpenAI Responses ────────────────────────────────────────────
EXTRACT_SCHEMA = {
    ...
    "schema": {
        "type": "object",
        "properties": {
            "expr_sympy":  {"type": "string", "description": "Expression in SymPy syntax, e.g., sin(x)**2/(x+1)"},
            "expr_latex":  {"type": "string", "description": "Expression in LaTeX, e.g., \\frac{\\sin^2 x}{x+1}"},
            "variable":    {"type": "string", "description": "Primary variable name, e.g., x"}
        },
        "required": ["expr_sympy"]
    }
}
...
def check_derivative():
    """
    JSON body:
      { "f_image": "data:image/png;base64,...",
        "g_image": "data:image/png;base64,...",
    ...
    try:
        # OCR both images
        f_obj = extract_expr_from_image(f_img, "This is the original function f(x).")
        g_obj = extract_expr_from_image(g_img, "This is the student's claimed derivative g(x).")

        f_sym = f_obj.get("expr_sympy", "")
        g_sym = g_obj.get("expr_sympy", "")
        f_var = (f_obj.get("variable") or varname).strip() or varname
        g_var = (g_obj.get("variable") or varname).strip() or varname
        # Use the requested/default var unless OCR very confidently changed it
        var = varname
        ...
        # Compute derivative with respect to var
        x = symbols(var)
        fprime = simplify(diff(f_expr, x))

        # First try exact/symbolic check
        symbolic_ok = simplify(fprime - g_expr) == 0

        # Then numeric robustness check
        ...
        # If incorrect, add a terse hint (model-generated) — optional
        if verdict == "incorrect":
            ...
            if hr.status_code < 400:
                ...
                hint_text = hj.get("output_text") or \
                    hj.get("output", [{}])[0].get("content", [{}])[0].get("text", {}).get("value")
                if hint_text:
...
@app.route("/")
def ui():
    # Minimal UI: two image inputs + variable
    return """
<!doctype html>
